...
class PickMlFusionTraceTests(unittest.TestCase):
    ...
    def test_trace_helper_failure_cannot_skip_prediction_or_change_recommendation(self):
        baseline = self._build_and_finalize()
        for helper in ("_trace_feature_names", "_trace_optional_float"):
            with self.subTest(helper=helper):
                previous_calls = self.model.predict_live_calls
                with patch.object(self.service, helper, side_effect=RuntimeError("private diagnostic detail")):
                    pick = self._build_and_finalize()
                self.assertEqual(self.model.predict_live_calls - previous_calls, 1)
                self.assertEqual(self._recommendation_projection(pick), self._recommendation_projection(baseline))
                self.assertEqual(pick["model_probability"], baseline["model_probability"])
                self.assertEqual(pick["model_version_id"], baseline["model_version_id"])
                self.assertEqual(pick["ml_fusion_trace"]["status"], "trace_error")
                self.assertNotIn("private diagnostic detail", str(pick["ml_fusion_trace"]))

    def test_nonserializable_trace_cannot_change_prediction_result(self):
        baseline = self._build_and_finalize()
        with patch.object(self.service, "_trace_feature_names", return_value=[object()]):
            pick = self._build_and_finalize()
        self.assertEqual(self._recommendation_projection(pick), self._recommendation_projection(baseline))
        self.assertEqual(pick["ml_fusion_trace"]["status"], "trace_error")
        self.assertEqual(pick["ml_fusion_trace"]["error_type"], "TypeError")

    def test_prediction_empty_and_failure_have_distinct_nonapplied_trace_status(self):
        with patch.object(self.model, "predict_live", return_value=None) as predict:
            pick = self._build_and_finalize()
        predict.assert_called_once()
        self.assertEqual(pick["ml_fusion_trace"]["status"], "prediction_unavailable")
        with patch.object(self.model, "predict_live", side_effect=RuntimeError("private token")) as predict:
            pick = self._build_and_finalize()
        predict.assert_called_once()
        self.assertEqual(pick["ml_fusion_trace"]["status"], "prediction_error")
        self.assertEqual(pick["ml_fusion_trace"]["error_type"], "RuntimeError")
        self.assertNotIn("private token", str(pick["ml_fusion_trace"]))
