...
from app.evaluation.full_market_ml.feature_materialization import (
    _peak_rss_bytes,
    certify_derived_feature_asset,
    materialize_feature_asset,
)
...
class FullMarketFeatureMaterializationTests(unittest.TestCase):
    ...
    def test_certifies_derived_contract_without_copying_or_rebuilding_matrix(self):
        with tempfile.TemporaryDirectory() as temporary:
            source = Path(temporary) / "source"
            parent = Path(temporary) / "parent-feature-asset"
            derived = Path(temporary) / "derived-feature-asset"
            data_path = source / "artifacts" / "full-build" / "dataset.parquet"
            data_path.parent.mkdir(parents=True)
            ...
            parent_manifest = materialize_feature_asset(
                source_dataset_root=source,
                output_root=parent,
                code_commit="fixture-parent",
                shard_count=2,
            )

            manifest = certify_derived_feature_asset(
                source_dataset_root=source,
                parent_feature_asset_root=parent,
                output_root=derived,
                code_commit="fixture-derived",
            )

            self.assertEqual(manifest["status"], "complete")
            self.assertTrue(manifest["coverage"]["passed"])
            self.assertTrue(manifest["online_offline_parity"]["passed"])
            self.assertEqual(manifest["matrix_path"], parent_manifest["matrix_path"])
            self.assertFalse((derived / "matrix").exists())
            self.assertEqual(
                certify_derived_feature_asset(
                    source_dataset_root=source,
                    parent_feature_asset_root=parent,
