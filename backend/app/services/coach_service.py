...
class CoachService:
    ...
    def _build_pick(
        ...
        # Observation errors must not enter the existing prediction/fusion handler.
        if ml_enrichment:
            try:
                ml_fusion_trace["model"].update({
                    "model_id": model_prediction.get("model_version_id"),
                    "model_version_id": model_prediction.get("model_version_id"),
                    "feature_schema": self._trace_feature_names(feature_payload),
                    "model_up_prob": self._trace_optional_float(model_probability.get("model_up_prob")),
                    "model_dd_prob": self._trace_optional_float(model_probability.get("model_dd_prob")),
                    "model_final_score": self._trace_optional_float(model_probability.get("final_score")),
                })
                ...
                json.dumps(ml_fusion_trace, allow_nan=False)
            except Exception as exc:
                ml_fusion_trace = {
                    "schema_version": "ml_fusion_trace_v2",
                    "status": "trace_error",
