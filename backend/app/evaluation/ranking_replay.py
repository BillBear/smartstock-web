...
def normalize_candidate_row(
    ...
    market_state = market_state or pick.get("market_state") or {}
    ...
    return {
        ...
        "source": source,
        "market_state_tag": str(market_state.get("state_tag") or "unknown"),
        "market_state_score": _safe_float(market_state.get("state_score"), 0.0),
        "was_bought": action_type in BUY_ACTION_TYPES,
        "buy_action_time": (action or {}).get("created_at") if action_type in BUY_ACTION_TYPES else None,
...
def attach_forward_labels(
    ...
    for row in rows or []:
        output = dict(row)
        try:
            history = data_source_manager.get_history_data(row.get("symbol"), days=90)
            future = _future_history(history, row.get("trade_date"))
...
class RankingReplayService:
    ...
    def _load_daily_candidates(self, strategy_code: str, risk_level: str, trade_date: str) -> List[Dict[str, Any]]:
        ...
        action_map = self._latest_action_map()
        rows = []
    ...
    def _store_snapshots(self, strategy_code: str, risk_level: str, trade_date: str) -> List[Dict[str, Any]]:
        ...
        if hasattr(self.store, "list_pick_snapshots"):
            return list(
                self.store.list_pick_snapshots(
        ...
        if hasattr(self.store, "engine"):
            return self._query_pick_snapshots(strategy_code=strategy_code, trade_date=trade_date)
        return []

    def _query_pick_snapshots(self, strategy_code: str, trade_date: str) -> List[Dict[str, Any]]:
        sql = text(
            """
            SELECT pick_id, user_id, trade_date, symbol, name, strategy_code, snapshot_json, created_at
            FROM pick_snapshots
    ...
    def _latest_action_map(self) -> Dict[str, Dict[str, Any]]:
        if not self.store:
            return {}
...
def _future_history(history: pd.DataFrame, trade_date: Any) -> pd.DataFrame:
    ...
    return rows[rows["date"] > str(trade_date)].sort_values("date").reset_index(drop=True)
