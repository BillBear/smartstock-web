...
def build_local_ml_config(payload: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "primary_label": str(payload.get("primary_label") or f"label_top20_{int(payload.get('primary_horizon') or 10)}d"),
        "candidate_set": str(payload.get("candidate_set") or "default"),
        "min_daily_panel_count": int(payload.get("min_daily_panel_count") or 0),
